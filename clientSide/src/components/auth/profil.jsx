...
const Profile = () => {
  ...
  function submit() {
    ...
    if (val.length < 1) {
      ...
      imgs.forEach((i) => {
        getInstance(token)
          .post(`http://${config.SERVER_HOST}:3000/images`, { img: i })
          .then((res) => {});
      });
      getInstance(token)
        .post(`http://${config.SERVER_HOST}:3000/infos`, {
          city,
          birthday,
          gendre: gender,
          sexpref: preferences,
      ...
      getInstance(token).post(`http://${config.SERVER_HOST}:3000/tags`, {
        tags: JSON.stringify(val),
      });
      getInstance(token)
        .post(`http://${config.SERVER_HOST}:3000/position`, { lat, lon })
        .then((res) => {});
