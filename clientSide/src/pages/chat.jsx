...
const Chat = () => {
  ...
  function updateMessages() {
    setTimeout(() => {
      getInstance(token)
        .get(
          `http://${config.SERVER_HOST}:3000/inbox/messages?user_id=${activeUser.id}&g_msgid=${showMessage.length}`
        )
        .then((res) => {
  ...
  function getMessage(user) {
    getInstance(token)
      .get(`http://${config.SERVER_HOST}:3000/inbox/messages?user_id=${user}`)
      .then((res) => {
        if (res.data.success) {
  ...
  function sendMessage(toLogin) {
    if (message.trim() === "" || message.length > 100) {
      ...
      getInstance(token)
        .post(`http://${config.SERVER_HOST}:3000/inbox`, {
          login: toLogin,
          msg: message,
        })
  ...
  useEffect(() => {
    getInstance(token)
      .get(`http://${config.SERVER_HOST}:3000/inbox/users`)
      .then((res) => {
            ...
            {userOnline?.map((e, index) => (
              ...
              >
                <ListItemIcon>
                  <Avatar
                    alt={e.login}
                    src={`http://${config.SERVER_HOST}:3000${config.SERVER_IMGS}${e.img}`}
                  />
                </ListItemIcon>
