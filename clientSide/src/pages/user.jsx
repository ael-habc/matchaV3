...
const socket = io.connect(`http://${config.SERVER_HOST}:3000`);
...
const User = () => {
  ...
  function reportUser() {
    if (token && user) {
      getInstance(token)
        .post(`http://${config.SERVER_HOST}:3000/report`, { login: user })
        .then((res) => {
          if (res.data.success) {
            setReport((old) => ({
              ...old,
  ...
  function blockUser() {
    if ((token, user)) {
      getInstance(token)
        .post(`http://${config.SERVER_HOST}:3000/blocks`, { login: user })
        .then((result) => {
  ...
  useEffect(() => {
    if (token) {
      getInstance(token)
        .get(`http://${config.SERVER_HOST}:3000/infos?login=${user}`)
        .then((res) => {
          if (res.data.error) {
            Swal.fire({
              icon: "error",
            ...
            const { birthday, city, desc, sex_pref, gendre, status } =
              res.data.data[0];
            setData(() => ({
              birthday: birthday,
              city: city,
      ...
      getInstance(token)
        .get(`http://${config.SERVER_HOST}:3000/users?login=${user}`)
        .then((res) => {
          if (res.data.success) {
      ...
      getInstance(token)
        .get(`http://${config.SERVER_HOST}:3000/tags?login=${user}`)
        .then((res) => {
          let active = [];
      ...
      getInstance(token)
        .get(`http://${config.SERVER_HOST}:3000/images?login=${user}`)
        .then((res) => {
          if (res.data.success) {
      ...
      getInstance(token)
        .get(`http://${config.SERVER_HOST}:3000/report?login=${user}`)
        .then((res) => {
          if (res.data.success) {
            setReport((old) => ({
              ...old,
       ...
       */
      getInstance(token)
        .get(`http://${config.SERVER_HOST}:3000/rating?login=${user}`)
        .then((res) => {
          setRating(res.data.rating);
        });
      getInstance(token)
        .get(`http://${config.SERVER_HOST}:3000/follow?to_login=${user}`)
        .then((res) => {
          if (res.data.success) {
  ...
  function handleRating(value) {
    if (rating === 0) {
      getInstance(token)
        .post(`http://${config.SERVER_HOST}:3000/rating`, {
          rat: value,
          login: user,
        })
        .then((res) => {
      ...
      getInstance(token)
        .patch(`http://${config.SERVER_HOST}:3000/rating`, {
          rat: value,
          login: user,
        })
        .then((res) => {
  ...
  function insetLike() {
    if ((token, user)) {
      getInstance(token)
        .post(`http://${config.SERVER_HOST}:3000/follow`, {
          to_login: user,
        })
        .then((result) => {
  ...
  return (
    <div className="userProfile">
      <Avatar
        src={
          image.length
            ? `http://${config.SERVER_HOST}:3000${config.SERVER_IMGS}${image[0].image_path}`
            : ""
        }
        ...
        <div className="galleryInfo">
          {image?.map((e, index) => (
            <img
              ...
              className="gallery"
              alt="gallery"
              src={`http://${config.SERVER_HOST}:3000${config.SERVER_IMGS}${e.image_path}`}
            />
          ))}
        </div>
      </div>
