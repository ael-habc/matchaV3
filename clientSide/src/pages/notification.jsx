...
const Notification = () => {
  ...
  useEffect(() => {
    ...
    if (token) {
      configSocket();
      getInstance(token)
        .get(`http://${config.SERVER_HOST}:3000/notifications?limit=0`)
        .then((res) => {
          if (res.data.success) {
  ...
  function realTimeAdd() {
    if (token) {
      getInstance(token)
        .get(
          `http://${config.SERVER_HOST}:3000/notifications?g_ntsid=` +
  ...
  function deletNotif(id) {
    getInstance(token)
      .delete(`http://${config.SERVER_HOST}:3000/notifications`, {
        data: {
          nts_id: id,
        },
      })
  ...
  function LoadMore() {
    ...
    getInstance(token)
      .get(
        `http://${config.SERVER_HOST}:3000/notifications?limit=${ntfslist.length}`
      )
      .then((res) => {
        if (res.data.success) {
